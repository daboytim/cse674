...
void setInitCentroids (Vector2& vec, vector<Cluster>& clusters, int k) {
    cout <<"setting init centroids...";
    int i, count=0;
    int space = vec.entries.size()/k;
    for (i=0; count < k; i++) {
	if (i%space == 0) {
	    Cluster c;
	    vector<double> cent (vec.entries[i].wordCounts.size(), 0.0);
	    int j;
	    for (j=0; j<cent.size(); j++) {
	        cent[j] = vec.entries[i].wordCounts[j];
	    }
	    c.centroid =  cent;
...
bool assignToClosest (vector<Cluster>& clusters, int mode) {
	...
	for (j=0; j<clusters[i].entries.size(); j++) {
		...
		clusters[closest].entries.push_back (clusters[i].entries[j]);
	      counts[clusters[i].entries[j].articleId]++;
		cout <<clusters[i].entries[j].articleId <<':' <<counts[clusters[i].entries[j].articleId] <<':';
