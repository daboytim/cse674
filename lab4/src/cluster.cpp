...
void writeClusters (vector<Cluster>&, Vector1&);
...
int main(int argc, char* argv[]) {
    ...
    readVector1 (vec1, "vector1", numArticles);
    if (rtn != 0) {
...
void writeClusters (vector<Cluster>& clusters, Vector1& vec1) {
	...
	fprintf (fdClusters, "%s\n", clusters[i].toString().c_str());
        ...
        s += num;
        FILE* cluster;
        cluster = fopen (s.c_str(), "w");
        int j;
        for (j=0; j<clusters[i].entries.size(); j++) {
            V2Entry entry = clusters[i].entries[j];
            int id = entry.articleId;
            fprintf (cluster, "%s\n", vec1.entries[id-1].toString().c_str());
        }
        fclose(cluster);
    }
