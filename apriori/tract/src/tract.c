/*----------------------------------------------------------------------
  ...
  History : 1996.02.14 file created as apriori.c
            ...
            2011.11.29 transaction array function added (for lcm eclat)
----------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define VERSION     "version 1.4 (2011.09.30)         " \
                    "(c) 2011   Christian Borgelt"
...
/*----------------------------------------------------------------------
  Constants
----------------------------------------------------------------------*/
static WIIT   WTA_END  = {-1,0};/* sentinel for item instance arrays */
...
{                               /* --- read a transaction */
  ...
  do {                          /* read the transaction items */
    ...
    &&  (mode & TA_WEIGHT)) {   /* and to read transaction weights */
      ...
      if (*e || (e == b)) return base->err = E_TAWGT;
    ...
    if (base->mode & IB_WEIGHTS) { /* if the items carry weights */
      if (x->size >= n) {       /* if the transaction buffer is full */
        ...
        x->items[base->size = n] = WTA_END; base->tract = x;
...
static void _pksort (TRACT **tracts, TRACT **buf, int n, int o)
{                               /* --- sort packed items with binsort */
  ...
  if (n < 64) {                 /* sort few transactions plainly */
...
static void _sort (TRACT **tracts, int n, int o,
...
{                               /* --- sort trans. with bucket sort */
  ...
  if (cnts[x] >= n) {           /* check for only one or no item */
    ...
    if ((x < 0) && (mask <= INT_MIN)) _pksort(tracts, buf, n, o);
    _sort(tracts, n, o+1, buf, cnts, k, mask);
    if ((x < 0) && (mask >  INT_MIN)) _pksort(tracts, buf, n, o);
  ...
  if ((*tracts)->items[o] < 0){ /* if there are packed items, sort */ 
    _pksort(tracts, buf, m = cnts[1] -m, o);
    if (mask <= INT_MIN) {      /* if to treat packed items equally */
      _sort(tracts, m, o+1, buf, cnts, k, mask);
...
int taa_reduce (TRACT **tracts, int n, int item,
                int *marks, void *buf, void *dst)
{                               /* --- reduce a transaction array */
  int   i, k;                   /* loop variables, comparison result */
  TRACT *s, **d;                /* to traverse the transactions */
  int   *p, *r;                 /* to traverse the item arrays */

  assert(tracts && (n > 0)      /* check the function arguments */
  &&    (k > 0) && marks && buf && dst);
  s = (TRACT*)dst;              /* get location for 1st transaction */
  for (i = k = 0; i < n; i++) { /* traverse the transactions */
    s->wgt = tracts[i]->wgt;    /* copy the transaction weight */
    p = tracts[i]->items;       /* get source and destination */
    r = s->items;               /* and filter the packed items */
    if ((*p < 0) && (*p > TA_END))
      if ((*r = *p++ & marks[0]) > TA_END) r++;
    for ( ; (unsigned)*p < (unsigned)item; p++)
      if (marks[*p]) *r++ = *p; /* filter the other items and */
    s->size = r -s->items;      /* compute the number of entries */
    if (s->size <= 0) continue; /* eliminate empty transactions */
    tracts[k++] = s;            /* replace the filtered transaction, */
    *r++ = TA_END;              /* store a sentinel at the end, */
    s = (TRACT*)r;              /* and switch to the next location */
  }                             /* in the destination buffer */
  if ((n = k) <= 0) return 0;   /* check for an empty result */
  if (item < n)                 /* if bin sort is possible/favorable */
    _sort(tracts, n, 0, buf, (int*)((TRACT**)buf+n)+1, item, -1);
  else                          /* otherwise sort with merge sort */
    ptr_mrgsort(tracts, n, ta_cmp, NULL, buf);
  for (d = tracts, i = 0; ++i < n; ) {
    s = tracts[i];              /* traverse the sorted transactions */
    k = s->size -(*d)->size;    /* compare the size first and */
    if (k == 0)                 /* only if the size is the same, */
      k = ta_cmp(s, *d, NULL);  /* compare the items */
    if (k == 0)                 /* if the transactions are equal, */
      (*d)->wgt += s->wgt;      /* combine the transactions */
    else                        /* if transactions are not equal, */
      *++d = s;                 /* store the current transaction */
  }                             
  return ++d -tracts;           /* return the new number of trans. */
}  /* taa_reduce() */
...
{                               /* --- sort a transaction bag */
  ...
  if (bag->mode & IB_WEIGHTS) { /* if the items carry weights */
    ...
    if (k < n) {                /* if bin sort is possible/favorable, */
      ...
      _sort((TRACT**)bag->tracts, n, 0, buf, cnts, k, mask); }
...
{                               /* --- pack all transactions */
  ...
  bag->mode |= TA_PACKED;       /* set flag for packed transactions */
}  /* tbg_pack() */
...
{                               /* --- main function */
  ...
  n = tbg_recode(tabag, (int)supp, -1, INT_MAX, sort);
