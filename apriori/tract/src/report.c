/*----------------------------------------------------------------------
  ...
  History : 2008.08.18 item set reporter created in tract.[ch]
            ...
            2011.10.18 bug in function _fastchk() fixed (hdr/sep check)
...
static void _fastchk (ISREPORT *rep)
...
static int _getsd (const char *s, const char **end)
...
int _mantout (ISREPORT *rep, double num, int digits, int ints)
...
{                               /* --- print an item weight */
  ...
  for (s = rep->iwfmt; *s; ) {  /* traverse the output format */
    ...
    t = s++; k = _getsd(s,&s);  /* get the number of signif. digits */
...
{                               /* --- create an item set reporter */
  ...
  rep->iwfmt  = ":%2w";         /* note header/separator/implication */
...
{                               /* --- open an output file */
  ...
  _fastchk(rep);                /* check for fast output */
  return 0;                     /* return 'ok' */
}  /* isr_open() */
...
static void _fastout (ISREPORT *rep, int n)
{                               /* --- fast output of an item set */
  char       *s;                /* to traverse the output buffer */
  const char *name;             /* to traverse the item names */

  assert(rep);                  /* check the function argument */
  if (rep->cnt > 0) {           /* unless this is the empty set */
    ...
    isr_putsn(rep, rep->info, rep->size);
  }                             /* print item set and its support */
  while (n > 0) {               /* traverse the perfect extensions */
    rep->items[rep->cnt++] = rep->pexs[--n];
    _fastout(rep, n);           /* add the next perfect extension, */
...
static void _report (ISREPORT *rep, int n)
{                               /* --- recursively report item sets */
  ...
  while (n > 0) {               /* traverse the perfect extensions */
    ...
    &&  (rep->cnt   <  rep->max))  /* (in the interval [min, max]), */
      _report(rep, n);             /* recursively report supersets */
...
{                               /* --- report the current item set */
  int    n, k;                  /* number of perfect exts., buffer */
  long   m, c, z;               /* buffers for item set counting */
  ...
  if (rep->mode & ISR_NOEXPAND){/* if not to expand perfect exts. */
    ...
    if (rep->fast) _fastout(rep, 0); /* report the expanded set */
    else           _output (rep);    /* (fast or normal output) */
  ...
  if (rep->fast) {              /* if fast output is possible */
    if ((rep->cnt <= 0)         /* format the item set support */
    &&  (rep->min <= 0)) {      /* handle empty set specially */
      isr_putsn(rep, rep->info, rep->size);
      rep->stats[0]++; rep->rep++;
    }                           /* count the empty set */
    _fastout(rep, n); }         /* report item sets recursively */
    ...
    _report(rep, n);            /* recursively add perfect exts. and */
