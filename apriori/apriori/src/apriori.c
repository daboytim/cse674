...
#define VERSION     "version 5.67 (2012.01.23)        " \
...
{                               /* --- apriori algorithm */
  ...
  for (i = n; 1; ) {            /* traverse the item set sizes */
    ...
    &&   (i < n) && (i *(double)tt < filter *n *tc))) {
      ...
      if (a.tatree) {           /* if a transaction tree was created */
        ...
        tbg_reduce(tabag);      /* transactions and reduce */
  ...
  XMSG(stderr, "[%ld %s(s)] done ", isr_repcnt(report),
               (n == ISR_RULE) ? "rule" : "set");
  XMSG(stderr, "[%.2fs].\n", SEC_SINCE(t));
...
{                               /* --- main function */
  ...
  if (argc > 1) {               /* if arguments are given */
    ...
    printf("-t#      test statistic for item set evaluation   "
                    "(default: '%c')\n", stat);
    printf("-p#      significance level (maximum p-value)     "
                    "(default: %g%%)\n", siglvl);
    ...
    if ((*s == '-') && *++s) {  /* -- if argument is an option */
      while (*s) {              /* traverse options */
        switch (*s++) {         /* evaluate switches */
          ...
          case 't': stat   = (*s) ? *s++ : 'x';     break;
          case 'p': siglvl =      strtod(s, &s);    break;
