...
int readVector1 (Vector1&, string);
...
int readVector1 (Vector1& vec1, string fileName) {
    ...
    int charRead;
    //fgets (buffer, BUFFER_SIZE, v1f);
    while (fgets (buffer, BUFFER_SIZE, v1f) != NULL) {
	V1Entry entry;
	int i;
	//get article ID
	char id[11];
...
int readVector2 (Vector2& vec2, string fileName, int lines) {
...
int Vector1::transactional() {
	...
	if (entry.words.size() > 0) {
		...
		word = entry.words[j];
		count = entry.wordCounts[j];
		fprintf (v1f, " %s", word.c_str(), count);
	    }
	}
	fprintf (v1f, "\n", NULL);
    }
